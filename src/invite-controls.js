...
function groupedEntries(pairs, groupSize = 5) {
  ...
  const blocks = [];
  for (const [email, links] of byEmail.entries()) {
    for (let i = 0; i < links.length; i += groupSize) {
      blocks.push({ email, links: links.slice(i, i + groupSize) });
    }
  }
  return blocks;
}
...
function copyPanelHtml(pairs) {
  const blocks = groupedEntries(pairs, 5);
  ...
  const cards = blocks.map((block, index) => {
    ...
    return `<div class="copy-email-block">
      ...
      <textarea id="copy-block-${index}" class="copy-block-text" readonly spellcheck="false">${escapeHtml(text)}</textarea>
  ...
  return `<div id="copy-links-panel" class="card stack">
    <div class="copy-panel-head">
      <div>
        ...
        <p class="tiny muted" style="margin:4px 0 0">Cada bloco mostra o e-mail da conta e até 5 links.</p>
      </div>
...
<style id="invite-controls-css">
  ...
  .copy-email-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px}
  ...
  .copy-block-text{width:100%;min-height:118px;resize:vertical;border:1px solid rgba(70,45,38,.18);border-radius:0;padding:10px 11px;font:11px/1.5 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;background:rgba(255,255,255,.62);color:inherit;white-space:pre;overflow:auto}
